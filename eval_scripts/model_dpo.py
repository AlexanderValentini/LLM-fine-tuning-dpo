...
from transformers import AutoModelForCausalLM, AutoTokenizer, AutoModelForSeq2SeqLM, PreTrainedModel
...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model: PreTrainedModel, **kwargs):
        r"""
        ...
        self.pretrained_model = pretrained_model  # Simply added for the type hinting
        ...
        # =========================================================================================
        self.custom_module =  pretrained_model
        ...
        ###########################################################################################
        # custom_module_kwargs, _, _ = self._split_kwargs(kwargs)
        # self.custom_module =  self.from_pretrained()
        # AdapterLayer(self.pretrained_model.config, **custom_module_kwargs)
        # self._init_weights(**custom_module_kwargs)
    ...
    def _init_weights(self, **kwargs):
        ...
        """
        strategy = kwargs.get("custom_module_init_strategy")
        if strategy is None:
            pass
        elif strategy == "normal":
            for module in self.custom_module.modules():
                if "weight" in dir(module) and isinstance(module.weight, torch.Tensor):
                    nn.init.normal_(module.weight)
        else:
            raise ValueError("Strategy can only be None or 'normal'")
...
#Here we get the output dictionary from the model:
...
#Outcommented this part as it gave errors for mcqa and is not needed:

#        last_hidden = output_dict["hidden_states"][-1]  # Last hidden states
#        output_hidden = self.custom_module(last_hidden)
#        output_dict["hidden_states"].append(output_hidden)
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        for prompt, chosen, rejected in zip(prompts, chosen_responses, rejected_responses, strict=True):
            
            #Here I took inspiration from the previous assignments - padding is set to the longest element
            #in the sequence to save memory.

            chosen_input = tokenizer(prompt + chosen, return_tensors="pt", padding="longest", truncation=True)
            #print("chosen_input", chosen_input)

            rejected_input = tokenizer(prompt + rejected, return_tensors="pt", padding="longest", truncation=True)

            #I try to move the input to the device in order to speed up computations:
            #This has to be done by iterating through the dictionary - as this is the output from the tokenizer.

            chosen_input = {k: v.to(self.pretrained_model.device) for k, v in chosen_input.items()}
            rejected_input = {k: v.to(self.pretrained_model.device) for k, v in rejected_input.items()}

            #Important to use torch.no_grad() to avoid memory issues (as we are evaluating):
            with torch.no_grad():
                chosen_output = self.pretrained_model(**chosen_input)
                rejected_output = self.pretrained_model(**rejected_input)

            chosen_log_probs = F.log_softmax(chosen_output.logits, dim=-1)
            rejected_log_probs = F.log_softmax(rejected_output.logits, dim=-1)

            prompt_token_ids = tokenizer(prompt, return_tensors="pt")["input_ids"][0]
            prompt_length = len(prompt_token_ids)
            #Is is important to only evaluate based on the completions (responses) and not the prompt.

            chosen_token_ids = chosen_input["input_ids"][0][prompt_length:]
            rejected_token_ids = rejected_input["input_ids"][0][prompt_length:]

            chosen_log_probs = chosen_log_probs[0, prompt_length:prompt_length + len(chosen_token_ids), :]
            rejected_log_probs = rejected_log_probs[0, prompt_length:prompt_length + len(rejected_token_ids), :]
            #Here I took inspiration with the gather function from Assignment 3:

            chosen_token_logps = torch.sum(torch.gather(chosen_log_probs, 1, chosen_token_ids.unsqueeze(1)).squeeze(1))
            rejected_token_logps = torch.sum(torch.gather(rejected_log_probs, 1, rejected_token_ids.unsqueeze(1)).squeeze(1))

            chosen_logps.append(chosen_token_logps)
            rejected_logps.append(rejected_token_logps)
    ...
    def prediction_step_reward(
        ...
        BETA = 0.1  # From the paper, could be something else (also mentioned 0.5)

        # Implementation inspired by paper
        def compute_reward(chosen_log_probs: torch.Tensor, ref_log_probs: torch.Tensor):
            return (BETA * (chosen_log_probs - ref_log_probs)).detach().tolist()

        output_dict["chosen_rewards"] = compute_reward(policy_chosen_logps, reference_chosen_logps)
        output_dict["rejected_rewards"] = compute_reward(policy_rejected_logps, reference_rejected_logps)
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        # Get letter choices as predictions
        ALL_CHOICES = ("A", "B", "C", "D")
        choice_tokens = tuple(tokenizer.encode(choice, add_special_tokens=False)[0] for choice in ALL_CHOICES)
...
#            choices = options.split("\n")
            choices = [option.split('.')[0] for option in options.split("\n") if option and "Answer" not in option]

            # Flattening
            if len(choices) % 4 == 0 and len(choices) > 4:
                print("BEFORE FLATTEN:", choices, len(choices))
                new_choices = []
                for i in range(0, len(choices), len(choices) // 4):
                    new_choices.append("\n".join(choices[i:i+(len(choices) // 4)]))
                choices = new_choices
                del new_choices
                del i
            ...
            # Sanity checks
#            assert len(choices) >= 2, "Too few choices provided"
#            assert len(choices) <= 4, "Too many choices provided"
            assert all(bool(x.strip()) for x in choices), "Some values are empty"

            # Prepare the input for the model
#            input_text = question_part + "\n" + "\n".join(choices) + f"\nAnswer: (last token generated should be in range 'A' - '{chr(ord('A') + (len(choices)-1))}'):"
            input_text = question + "Please output a single letter corresponding to the correct answer"#+ "\n" f"(last token generated should be in range 'A' - '{chr(ord('A') + (len(choices)-1))}'):"

            chat = [{"role": "user", "content": input_text}]
            input_ids = tokenizer.apply_chat_template(chat, return_tensors="pt", add_generation_prompt = True)
            input_ids = input_ids.to(next(self.parameters()).device)

            # Forward pass through the model
            with torch.no_grad():
                log_probs = F.log_softmax(self.forward(input_ids=input_ids).logits[:, -1, :], dim=-1)

            # Parse prediction
            prediction = torch.argmax(log_probs, dim=-1).item()
            assert isinstance(prediction, int), "Prediction is not an integer!"
            prediction_token = tokenizer.decode([prediction]).strip()

            # Prediction token is valid
            if prediction_token in ALL_CHOICES[:len(choices)]:
                 print("PREDICTION VALID:", prediction_token)
                 output_dict["preds"].append(prediction_token)
                 continue

            # Use probabilistic approach
            prediction = torch.argmax(log_probs[0, choice_tokens[:len(choices)]], dim=-1).item()
            assert isinstance(prediction, int), "Prediction is not an integer!"
            assert prediction >= 0 and prediction < len(choices), "Prediction not in range!"

            # Add prediction as letter
            print("Prediction:", chr(ord("A") + prediction))
            output_dict["preds"].append(chr(ord("A") + prediction))
...
class AutoDPOModelForSeq2SeqLM(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        #Outcommented this since I dont use seq2seq model:
        #custom_module_kwargs, _, _ = self._split_kwargs(kwargs)
        #self.custom_module = model
        #self._init_weights(**custom_module_kwargs)
    ...
    def _init_weights(self, **kwargs):
        ...
        Args:
            **kwargs (`dict`, `optional`):
                Additional keyword arguments, that are passed to the `CustomModule` class.
        """
#Outcommented this since we dont use seq2seq model:
#        strategy = kwargs.get("custom_module_init_strategy")
#        if strategy is None:
#            pass
#        elif strategy == "normal":
